...
class BoardingSystemApp:
    ...
    def add_routes(self):
        ...
        def checkin_passenger(passenger_id: int, db: Session = Depends(self.get_db)):
            # 1. Encontra o passageiro no banco de dados
            passenger = db.query(self.PassengerDB).filter(self.PassengerDB.id == passenger_id).first()
            ...
            # 3. Atualiza o status do check-in
            passenger.CHECKIN_STATUS = 1
            
            # 4. Salva (commita) a alteração no banco
            try:
                db.commit()
                db.refresh(passenger) # Atualiza o objeto com os dados do banco
                return passenger
            except:
                db.rollback()
